...
export class D1TodoRepository implements ITodoRepository {
    ...
    async findByUserId(
        input: FindTodosInput & PaginationParams,
    ): Promise<PaginatedResult<TodoWithRelations>> {
        ...
        if (input.categoryId)
            conditions.push(eq(todos.categoryId, input.categoryId));
        if (input.priority) conditions.push(eq(todos.priority, input.priority));
        if (input.search)
            conditions.push(like(todos.title, `%${input.search}%`));
        ...
        if (column === "createdAt")
            ...
            orderBy =
                direction === "desc" ? desc(todos.dueDate) : asc(todos.dueDate);
        ...
        if (input.tagId) {
            mapped = mapped.filter((t) =>
                t.tags.some((tag) => tag.id === input.tagId),
            );
            total = mapped.length;

// @ts-ignore — Cloudflare Workers compat
const FIREBASE_API_KEY =
    (
        globalThis as unknown as {
            process?: { env?: { FIREBASE_API_KEY?: string } };
        }
...
async function seed() {
    ...
    for (const user of SEED_USERS) {
        try {
            ...
            console.log(
                `  ✅ ${user.email} (uid: ${firebaseUid}, role: ${user.role})`,
            );
            console.log(
                "     → Run D1 INSERT to add this user to the database",
            );
        } catch (err) {
